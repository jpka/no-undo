...
/**
 * Render a judge-legible invoice HTML from the EsignPayload.
 ...
 *
 * Sources line items, vendor, payer, and totals from the shared invoice
 * fixture (mcp/fixtures/invoice-data.mjs) when payload.invoice is omitted —
 * so the Foxit-assembled PDF and the Nutrient-extracted document describe
 * the same figures.
 *
 * @param {{
 *   folderName: string,
 ...
 *   invoice?: {invoiceNumber:string,title:string,vendor:{name:string,location:string},payer:{name:string,location:string},receivedDate:string,lineItems:Array<{description:string,quantity:number,unitPrice:number,total:number}>,subtotal:number,taxRate:number,taxAmount:number,totalDue:number}
 * }} payload
 * @returns {string} HTML string
 */
export function buildInvoiceHtml(payload) {
  const invoice = payload.invoice ?? INVOICE;
  const title = escapeHtml(payload.folderName || invoice.title || "Document");
  ...
  // Line items from the shared fixture — same figures the Nutrient stage extracts.
...
<body style="font-family:Inter,system-ui,Arial,sans-serif;margin:40px;color:#111;background:#fff;">
  ...
  <p style="color:#555;margin:0 0 16px 0;font-size:13px;">Invoice #${escapeHtml(invoice.invoiceNumber)} — Generated by No Undo — Foxit PDF Services (pdf_from_html) before the approval gate.</p>
  ${docSource}
  <p style="margin:16px 0 8px 0;"><strong>Instructions:</strong> ${instructions}</p>
  <div style="margin:16px 0;font-size:13px;">
    <div><strong>Vendor:</strong> ${escapeHtml(invoice.vendor.name)} — ${escapeHtml(invoice.vendor.location)}</div>
  ...
  <table style="border-collapse:collapse;width:100%;font-size:13px;margin-top:12px;">
    <tbody>
      <tr><td style="padding:8px;border:1px solid #ddd;text-align:right;"><strong>Subtotal</strong></td><td style="padding:8px;border:1px solid #ddd;text-align:right;">${escapeHtml(usd(invoice.subtotal))}</td></tr>
      <tr><td style="padding:8px;border:1px solid #ddd;text-align:right;"><strong>Tax (${escapeHtml(String(invoice.taxRate))}%)</strong></td><td style="padding:8px;border:1px solid #ddd;text-align:right;">${escapeHtml(usd(invoice.taxAmount))}</td></tr>
      <tr><td style="padding:8px;border:1px solid #ddd;text-align:right;"><strong>Total due</strong></td><td style="padding:8px;border:1px solid #ddd;text-align:right;">${escapeHtml(usd(invoice.totalDue))}</td></tr>
    </tbody>
  </table>
