...
function extractRecipients(prompt) {
  const emails = [...prompt.matchAll(EMAIL_RE)].map((m) => m[0]);
  if (emails.length > 0) {
...
function extractNamedRecipients(prompt) {
  ...
  for (const n of known) {
    if (lower.includes(n.first.toLowerCase())) {
...
function extractInstructions(prompt) {
  const lower = prompt.toLowerCase();
  const parts = [];
  if (lower.includes("redact")) parts.push("redact PII");
  if (lower.includes("sign")) parts.push("send for signature");
  if (lower.includes("review")) parts.push("review required");
  return parts.length > 0 ? parts.join("; ") : null;
}

function makeExcerpt(prompt) {
  if (prompt.length <= 120) return prompt;
  return prompt.slice(0, 120).trimEnd() + "…";
}
