...
5. **The gate** — `agent/esign-agent-loop.mjs` renders the plan (prompt excerpt, recipients, extraction routing, what was redacted and the confirmation it is gone, document SHA-256, irrevocability warning) and waits for human approval. The approval server binds an ephemeral localhost port and requires a per-session bearer token on every route (`safe-write-mcp-core` 0.4.0); the printed URL carries it as `?token=`.
