...
**Aug 26–28 — ~12h. Nutrient stage.** ✅ **DONE Aug 20, six days early — see `docs/nutrient-stage-aug20.md`.** The blocker cleared (Data Extraction key in `.env`), so extraction routing, staged redaction, the Nutrient MCP server, and 53 new tests all landed. 60/60 tests pass repo-wide.
...
**Aug 29–30 — ~8h. Audit and UI.** ~~1/3 done.~~ ~~Drop raw `payload` from `GET /api/plans` — right now a host's careful `renderPlan` redaction is silently bypassed by that endpoint, which is embarrassing in a PII demo.~~ **DONE Aug 20** via `safe-write-mcp-core@0.3.0` (issue #18, PR #19 — drops `payload` by default, opt-in via `exposeRawPayload`). Consumed here by version bump. Remaining: hash-chained JSONL sink with `prevHash`. Approval page that renders the document and the recipient list, not `JSON.stringify`.
